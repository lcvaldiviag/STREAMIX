...
const ChatBot = ({ visible = true }: ChatBotProps) => {
    ...
    const [messages, setMessages] = useState<{ role: 'user' | 'model'; text: string }[]>([
        { 
            role: 'model', 
            text: '¡Hola! Soy <b>MI_A</b> ✨. Tu consultora de élite.<br/><br/>¿Qué experiencia buscas activar hoy? 🚀' 
        }
    ]);
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            setMessages(prev => [...prev, { role: 'model', text: 'Lo siento, tuve un tropiezo técnico. Chatea conmigo en WhatsApp para atención VIP.' }]);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    return (
        <>
            {/* Chat Window - Reduced Width and Height */}
            {isOpen && (
            ...
            >
                 {isOpen ? (
                    ...
                    <>
                        ...
                        <span className="text-white font-black text-[9px] md:text-xs tracking-widest uppercase">ASESORÍA ✨</span>
                    </>
                 )}
            </button>
        </>
