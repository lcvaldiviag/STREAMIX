...
const MenuIcon = ({ isOpen }: { isOpen: boolean }) => (
    <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 transition-all" fill="none" viewBox="0 0 24 24" stroke="currentColor">
    ...
    return (
        <header className={`fixed top-0 left-0 right-0 z-[100] transition-all duration-500 ${scrolled ? 'py-2' : 'py-6'}`}>
            <div className={`mx-auto max-w-7xl px-4 sm:px-6 lg:px-8`}>
                <div className={`relative flex items-center justify-between px-6 h-16 lg:h-20 rounded-full transition-all duration-500 ${scrolled ? 'bg-white/80 dark:bg-slate-900/80 backdrop-blur-2xl border border-slate-200 dark:border-white/10 shadow-xl' : 'bg-transparent border-transparent'}`}>
                    <button onClick={onNavToggle} className="lg:hidden p-3 rounded-full text-slate-800 dark:text-white hover:bg-slate-100 dark:hover:bg-white/10 transition-colors">
                    ...
                    <div className="flex-1 max-w-xl mx-8 hidden md:block">
                        <div className="relative group">
                            ...
                            <input
                                type="text"
                                placeholder="Buscar en Streamix..."
                                value={searchQuery}
                                onChange={(e) => onSearchChange(e.target.value)}
                                className="w-full bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-full py-2.5 pl-12 pr-6 text-slate-900 dark:text-white placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:bg-white dark:focus:bg-white/10 transition-all shadow-sm"
                            />
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        <button 
                            ...
                            className="p-3 rounded-full bg-slate-100 dark:bg-white/5 text-slate-800 dark:text-white hover:scale-110 active:scale-95 transition-all shadow-sm border border-slate-200 dark:border-white/10"
                            title={isDarkMode ? "Cambiar a modo claro" : "Cambiar a modo oscuro"}
                        >
                            {isDarkMode ? <SunIcon /> : <MoonIcon />}
                        </button>
                        
                        <button onClick={onCartClick} className="relative p-3 rounded-full bg-indigo-600 text-white shadow-lg hover:scale-110 active:scale-95 transition-all">
                            <ShoppingCartIcon />
                            {cartItemCount > 0 && (
                                <span className="absolute -top-1 -right-1 flex h-6 w-6 items-center justify-center rounded-full bg-white text-indigo-600 text-[10px] font-black shadow-lg border border-indigo-100">
                                    {cartItemCount}
                                </span>
                            )}
                        </button>
